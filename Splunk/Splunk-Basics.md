...
The `lookup` command enriches Windows event logs by matching existing fields with information stored in an external lookup table. In this example, hostnames were matched to user roles to provide additional context during log analysis.
...
The lookup table successfully mapped each hostname to its corresponding user role. This enrichment provides additional context for investigations and helps analysts quickly identify the type of user associated with each system.

### Screenshot

![Lookup Command](lookup_command.png)

## 7.8 Eval Command

The `eval` command was used to create a new field called `LogonTypeDesc`, which converts numeric LogonType values into descriptive text. This makes Windows logon events easier to interpret during investigations.
...
The query translated numeric logon types into descriptive values and displayed the number of occurrences for each type. This improves readability and helps analysts quickly understand the meaning of authentication events.
...
- Converted LogonType values into descriptive names.
- Displayed the count of each logon type for easier analysis.

### Findings

- Enriched Windows events using an external lookup table.
- Mapped each hostname to its associated user role.
- Displayed the number of events generated by each host and role.
